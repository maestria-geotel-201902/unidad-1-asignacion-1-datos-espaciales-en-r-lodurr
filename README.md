...
## Reading layer `PROVCenso2010' from data source `/home/ingdi/unidad-1-asignacion-1-datos-espaciales-en-r-lodurr/data/divisionRD.gpkg' using driver `GPKG'
...
``` r
plot(prov[prov$PROV %in% '19', 'PROV'])
```
...
## 19   19  03 HERMANAS MIRABAL   0319 MULTIPOLYGON (((368431.6 21...
plot(miprov)
```
...
-   Genera una tabla de provincias sólo con las columnas `TOPONIMIA` y `areaenm2`, y ordénalas de mayor a menor usando el campo `areaenm2`. Aunque puedes ejecutar una tubería de manera fluida y en una sola línea, te recomiendo hacerlo paso a paso, para que explores el efecto de cada verbo y la funcionalidad del operador `pipe` (`%>%`).
    ...
    -   Organiza el resultado mediante la función `arrange`. Verás que la tabla se organizará ascendentemente. En caso de que quieras leer mas acerca de la funcion `arrange` puedes encontrar en este link en inglés mas información <https://www.datanovia.com/en/lessons/reorder-data-frame-rows-in-r/>
...
##          areaenm2
## 1 427029190 [m^2]
prov %>% dplyr::filter(PROV %in% '19') %>% dplyr::select('areaenm2') %>% plot
```
...
``` r
prov %>% dplyr::filter(PROV %in% c('19', '02', '17')) %>% dplyr::select('areaenm2') %>% plot
```
...
## Reading layer `MUNCenso2010' from data source `/home/ingdi/unidad-1-asignacion-1-datos-espaciales-en-r-lodurr/data/divisionRD.gpkg' using driver `GPKG'
...
``` r
mimun <- mun %>% dplyr::filter(PROV %in% '19')
mimun %>% plot
```
